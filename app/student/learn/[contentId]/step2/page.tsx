...
import type { ContentDoc, Keyword, ScoreDoc } from '@/lib/firebase/types';
...
export default function Step2Page() {
  ...
  useEffect(() => {
    ...
    getContentDoc(contentId).then(doc => {
      if (doc) {
        setContentDoc(doc);
        const rawKeywords = doc.step2_keywords_list || [];
        const normalizedKeywords = rawKeywords.map(kw => ({
          ...kw,
          keyword: normalizeText(kw.keyword),
          description: normalizeText(kw.description)
        }));
  ...
  const currentItem = keywords[currentIndex] || { keyword: '', description: '' };
  const targetText = currentItem.description;
  const currentLines = targetText ? splitIntoLines(targetText, 45) : [];
  const currentSentence = currentLines[currentLineIndex] || '';

  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    const val = e.target.value;
    const now = Date.now();
    if (!startTime && val.length > 0) setStartTime(now);
    setInputValue(val);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (e.key === 'Enter' || (e.key === ' ' && (isAtEnd || isPerfectMatch))) {
      ...
      } else if (currentIndex < keywords.length - 1) {
        if (startTime) {
  ...
  const finishStep = async () => {
    ...
    if (user?.uid && profile) {
      ...
      await upsertScore(contentId, user.uid, {
        ...
        step2_score: finalScore,
        step2_wpm: Math.round(currentWpm),
        step2_accuracy: Math.round(currentAccuracy),
        step2_completedAt: new Date() as any
      });
    }
  };

  if (loading) return <div style={{ padding: 20 }}>Loading...</div>;
  if (!contentDoc || keywords.length === 0) return <div style={{ padding: 20 }}>콘텐츠를 찾을 수 없습니다.</div>;

  if (isFinished) {
    const finalScore = currentAccuracy <= 50 ? 0 : Math.min(100, Math.round((currentWpm / 300) * currentAccuracy));
    const sortedScores = [...scores].sort((a, b) => (b.step2_score || 0) - (a.step2_score || 0));
      ...
      <div className="page-wrapper" style={{ background: 'var(--color-bg-secondary)', height: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <main className="container" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 'var(--spacing-lg)' }}>
          <div className="card" style={{ padding: 'var(--spacing-2xl)', textAlign: 'center', maxWidth: 500, width: '100%', boxShadow: 'var(--shadow-lg)' }}>
            ...
            <h1 style={{ fontSize: '1.5rem', fontWeight: 800, marginBottom: 'var(--spacing-lg)', color: 'var(--color-text-primary)' }}>2단계 핵심 어휘 학습 완료!</h1>
            ...
            <button className="btn btn-primary" style={{ width: '100%', padding: 'var(--spacing-md)', fontSize: '1.125rem' }} onClick={() => router.push(`/student/learn/${contentId}/step3`)}>
              3단계로 넘어가기 →
            </button>
          </div>
        </main>
      ...
      <main className="container" style={{ flex: 1, minHeight: 0, padding: 'var(--spacing-lg)', display: 'grid', gridTemplateColumns: '1fr 300px', gap: 'var(--spacing-lg)', maxWidth: 1300, margin: '0 auto', width: '100%' }}>
        <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-sm)', marginBottom: 'var(--spacing-md)', flexShrink: 0 }}>
            <span className="step-badge active" style={{ background: 'var(--color-primary)' }}>2</span>
            <h1 style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--color-text-primary)' }}>핵심 어휘 학습</h1>
            <span style={{ fontSize: '0.8125rem', color: 'var(--color-text-muted)' }}>— {contentDoc.unit}: {contentDoc.title}</span>
          </div>

          <div style={{ background: 'var(--color-primary-light)', border: '1px solid #BFDBFE', borderRadius: 'var(--radius-md)', padding: 'var(--spacing-sm) var(--spacing-md)', marginBottom: 'var(--spacing-md)', fontSize: '0.875rem', color: 'var(--color-primary)', display: 'flex', alignItems: 'center', gap: 'var(--spacing-xs)', flexShrink: 0 }}>
            <span>💡</span><span>이 단계는 개념 집중 학습 단계입니다. 순위 경쟁 없이 핵심 어휘를 차분히 익혀보세요.</span>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 'var(--spacing-sm)', fontSize: '0.875rem', color: 'var(--color-text-muted)', flexShrink: 0 }}>
            <span>어휘 {currentIndex + 1} / {keywords.length}</span>
          </div>
          <div className="progress-bar-track" style={{ marginBottom: 'var(--spacing-md)', background: 'var(--color-border-default)', flexShrink: 0 }}>
            <div className="progress-bar-fill" style={{ width: `${((currentIndex) / keywords.length) * 100}%`, background: 'var(--color-primary)' }} />
          </div>
          ...
          <div className="card" style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', padding: 'var(--spacing-xl)' }}>
            <div style={{ marginBottom: 'var(--spacing-md)', flexShrink: 0 }}>
              <span style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>핵심 어휘</span>
              <p style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--color-primary)', marginTop: 4 }}>{currentItem.keyword}</p>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 'var(--spacing-lg)', paddingTop: 'var(--spacing-md)', flexShrink: 0 }}>
              <div style={{ display: 'flex', gap: 'var(--spacing-md)', alignItems: 'center' }}>
                ...
                {!startTime && currentIndex > 0 && !isFinished && (
                  <div style={{ marginLeft: 'var(--spacing-md)', color: 'var(--color-text-secondary)', fontSize: '0.875rem' }}>
                    타이핑을 시작하면 타수가 다시 측정됩니다.
                  </div>
                )}
              </div>
              <button className="btn btn-primary" onClick={() => router.push(`/student/learn/${contentId}/step3`)}>
                건너뛰기 →
              </button>
            </div>
          </div>
        ...
        <aside style={{ height: '100%', overflowY: 'auto' }}>
          <div className="card" style={{ padding: 'var(--spacing-md)' }}>
            <LeaderboardSlide scores={scores} step={2} />
          </div>
        </aside>
      </main>
    </div>
